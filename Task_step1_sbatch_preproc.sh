...
###??? update these
parDir=~/fsl_groups/fslg_KirwanLab/compute/Duraccio_RAD 
workDir=${parDir}/derivatives/$subj
dataDir=${parDir}/rawdata/$subj

tempDir=${parDir}/code/Templates/vold2_mni
template=${tempDir}/vold2_mni_brain+tlrc
priorDir=${tempDir}/priors_ACT

blip=0										# blip toggle (1=on)

phaseArr=(gonogo)					# Each PHASE of experiment, in order (TEST1 precedes TEST2 but followed STUDY). This is absolutely necessary - so put something here
